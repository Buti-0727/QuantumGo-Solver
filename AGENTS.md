...
When rules, algorithms, or design decisions conflict, resolve in this priority order:

1. [`QuantumGo Solver Coding Protocol.md`](./QuantumGo%20Solver%20Coding%20Protocol.md) *(Highest priority)*
2. [`prd/`](./prd/) files (`PRD-01` through `PRD-05`)
3. [`quantumgo_solver_capability_specification.md`](./quantumgo_solver_capability_specification.md)
4. [`quantumgo_solver_agent_development_phases.md`](./quantumgo_solver_agent_development_phases.md)
...
| Area | Rule / Invariant |
| :--- | :--- |
| **Boards** | Exactly two boards ($B_1$ and $B_2$). |
| **Moves 1 & 2** | Move 1 is Black (one stone on $B_1$, one on $B_2$). Move 2 is White (one on $B_1$, one on $B_2$). Stored as entangled pairs. |
| **Move 3+** | Played at identical coordinate $(x, y)$ on both $B_1$ and $B_2$. Illegal if illegal on either board. |
| **Captures** | Capturing a stone removes its entangled partner on the opposite board immediately. |
| **Cascades** | Partner removals can reduce neighboring liberties to 0, triggering recursive capture cascades. Must resolve to stable state. |
| **Relevance Zone ($RZ_Q$)** | $RZ_Q = RZ_{B1} \cup RZ_{B2} \cup \text{EntangledPartners}(RZ)$. Expands when tactical moves touch boundaries. |
| **Search Tree** | Exact AND/OR proof tree search; terminal states must definitively evaluate ALIVE, DEAD, SEKI, or KO. |

---

## 4. Phased Development Workflow

Work strictly in the sequence outlined in [`quantumgo_solver_agent_development_phases.md`](./quantumgo_solver_agent_development_phases.md):
